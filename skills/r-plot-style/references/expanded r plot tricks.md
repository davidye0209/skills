...
author: "Yan Holtz"
site: "Yan Holtz"
domain: "r-graph-gallery.com"
url: "https://r-graph-gallery.com/web-lineplots-and-area-chart-the-economist.html"
description: "This post explains how to build and combine customized lineplots and stacked area charts with R and ggplot2. Step by step code snippets with explanations are provided."
---
...
Thanks to them for all the inspiring and insightful visualizations! Thanks also to [Tomás Capretto](https://tcapretto.netlify.app/) who replicated the chart in R! 🙏🙏

As a teaser, here is the plot we’re gonna try building:

![](https://www.economist.com/img/b/1000/591/90/sites/default/files/images/2021/08/articles/main/20210828_woc033.png)
...
This is article uses several plotting libraries apart from the nice `ggplot2` we always use. The first one is [`shadowtext`](https://github.com/GuangchuangYu/shadowtext), a library that allows to draw text with shadows. Then, the popular [`patchwork`](https://patchwork.data-imaginist.com/), which is going to make the task of combining `ggplot2` figures extremely easy. And finally, we’re also going to use the `grid` library, the drawing library behind `ggplot2`.

In addition, other utilities libraries are used: [`ggtext`](https://wilkelab.org/ggtext/) to draw text with multiple styles very easily, and [`ggnewscale`](https://eliocamp.github.io/ggnewscale/) to use multiple color scales in the same `ggplot2` plot.
...
The chart we’re going to reproduce today is made of two separated plots, a [line chart](https://www.r-graph-gallery.com/line-plot.html) and a [stacked area chart](https://www.r-graph-gallery.com/stacked-area-graph.html). These charts use different datasets that are created below:
...
![](https://r-graph-gallery.com/web-lineplots-and-area-chart-the-economist_files/figure-html/unnamed-chunk-5-1.png)  
...
![](https://r-graph-gallery.com/web-lineplots-and-area-chart-the-economist_files/figure-html/unnamed-chunk-6-1.png)  
...
![](https://r-graph-gallery.com/web-lineplots-and-area-chart-the-economist_files/figure-html/unnamed-chunk-7-1.png)  

## Stacked area chart

---

Thanks to the `geom_area()` function, it is quite straightforward to create a [stacked area chart](https://www.r-graph-gallery.com/stacked-area-graph.html) in `ggplot2`.
...
![](https://r-graph-gallery.com/web-lineplots-and-area-chart-the-economist_files/figure-html/unnamed-chunk-8-1.png)  
...
![](https://r-graph-gallery.com/web-lineplots-and-area-chart-the-economist_files/figure-html/unnamed-chunk-9-1.png)  

## Add labels and annotations

---

We’re about to finish this chart. This last step, which may be the most convoluted step, consists of adding several labels and various annotations. Firs of all, the labels for the areas. Then, the labels for the horizontal gridlines. After that, we make use of the `geom_curve()` function to add a curve that goes from the “Latin America and the Caribbean” label to the area it represents. And finally, it’s time for a title.
...
![](https://r-graph-gallery.com/web-lineplots-and-area-chart-the-economist_files/figure-html/unnamed-chunk-10-1.png)  

## Combine charts

---

Now it comes one of the most exciting steps: combining the charts! Thankfully, it exists `patchwork` which makes it extremely easy to combine plots made with `ggplot2`.
...
Next, we add a title to the plot obtained with patchwork using `plot_annotation()` and a theme created for it.
...
![](https://r-graph-gallery.com/web-lineplots-and-area-chart-the-economist_files/figure-html/unnamed-chunk-11-1.png)  
...
The chart above is quite a good replicate of the original figure, but it is clearly missing some small, but very important, details. These are the distinctive red marks on top, and the captions with information about the source of the data and credit to the original author.

We’re going to use the `grid` library for this last task. `grid` is a low-level plotting library that comes with any `R` installation by default and provides many plotting primitive functions. It is also the library that `ggplot2` uses to create the charts under the hood, and that’s why we can combine them in the same chart.

Using `grid` will give us full control of what is added and where it is added to the plot. The downside, is that it requires us to write a considerable amount of extra code.
...
![](https://r-graph-gallery.com/web-lineplots-and-area-chart-the-economist_files/figure-html/unnamed-chunk-12-1.png)  
...
One alternative, the one we used [here](https://www.r-graph-gallery.com/web-horizontal-barplot-with-labels-the-economist.html), is to remove the titles made with `ggplot2` and make all the annotations from scratch using the `grid` library. Do you like challenges? Then have a look at the article and go for it!
